...
class ProfileService {
    ...
    createProfile = async (userId: string, profileData: CreateProfileData): Promise<UserProfile | null> => {
        try {
            let avatarUrl = null;

            // 1. Upload Avatar if present
            if (profileData.avatar_uri && !profileData.avatar_uri.startsWith('http')) {
                const extension = profileData.avatar_uri.split('.').pop()?.toLowerCase() || 'jpg';
                const path = `${userId} /avatar.${extension}`;
                avatarUrl = await storageService.uploadImage(profileData.avatar_uri, 'avatars', path);
            }
