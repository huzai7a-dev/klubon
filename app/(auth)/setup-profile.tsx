...
export default function SetupProfileScreen() {
    ...
    const isCurrentStepValid = () => {
        ...
        switch (currentStep) {
            case 0:
                ...
                break;
            case 1:
                isValid = activitySelectionSchema.safeParse({ activities: data.activities }).success;
                break;
            case 2:
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            ...
            isStepValid = await trigger(["prefers_male", "prefers_female", "prefers_other"]);
            if (isStepValid) {
                const data = methods.getValues();
                const prefData = {
                    prefers_male: data.prefers_male,
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            if (profile) {
                // Refresh session/profile in context if needed, or just navigate
                // Assuming createProfile success means we are good.
                // Ideally we might want to reload the profile in AuthContext here:
                // await loadProfile(); // if exposed, or just rely on next protected route check
